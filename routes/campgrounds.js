...
router.route('/')
    ...
    .post(isLoggedIn, upload.array('image'), validateCampground, catchAsync(campgrounds.createNewCamp)); //image here is the name given to the input type on the form //where it submits to(this should be only possible for who is logged in)
...
router.route('/:id')
    ...
    .put(isLoggedIn, isAuthor, upload.array('image'), validateCampground, catchAsync(campgrounds.updateCamp)) //where to submit to(this should be only possible for who is logged in)
